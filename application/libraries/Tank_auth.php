...
class Tank_auth
{
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			//echo '<pre/>';
			//die(var_dump($this->ci->users->$get_user_func($login)));
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if(array_key_exists('banned',  $user)){
	...
	function forgot_password($login)
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->users->get_user_by_login($login))) {
				...
				$this->ci->users->set_password_key($user->_id, $data['new_pass_key']);
				return $data;
		...
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0) AND (strlen($new_password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->get_user_by_id($user_id,  TRUE))) {
				...
				if ($this->ci->users->reset_password(
					...
					$this->ci->user_autologin->clear($user->_id);
	...
	function login_check(){
		...
		if(!$this->is_logged_in()){
			...
			$current_url = $this->ci->session->set_userdata('current_url',  current_url());
			redirect('/auth/login/');	
			}else{
			if($this->ci->session->userdata('current_url')){
				$this->ci->session->unset_userdata('current_url');
			}
		}
		
		//echo('<script>location.href = "http://localhost/grdc/auth/login/"</script>');
	}
